...
LEFT_TO_ENGINE = "  %d codepoints already in Godot's own font" + chr(10)


def builtin_coverage() -> set:
    """Codepoints Godot's built-in font already provides.

    Generated by tools/dump-builtin-font-coverage.gd. A missing file is not
    an error: the gate goes back to requiring everything, which is safe and
    only wasteful.
    """
    path = os.path.join(HERE, "builtin-font-coverage.txt")
...
def build(check_only: bool) -> int:
    ...
    # What the engine can already draw is not this font's job. The web build
    # falls through to Godot's built-in face when SystemFont finds no
    # operating system to ask, and that face carries Latin Extended-A among
    # much else.
    #
    # Subtracting it is accuracy, not optimisation. While this gate demanded
    # that the JAPANESE subset also cover c-acute, it reported a failure that
    # was not real -- and I renamed a character to satisfy it instead of
    # checking whether the glyph could be drawn at all. A gate that fails
    # wrongly gets obeyed wrongly.
    _builtin = builtin_coverage()
    _shared = len(cps & _builtin)
    cps = cps - _builtin
    if _shared:
        sys.stderr.write(LEFT_TO_ENGINE % _shared)
