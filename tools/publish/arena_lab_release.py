"""Stage the C.09.1 public cabinet from an exact, clean source checkout.
...
def stage(source, deployed_manifest, output, commit):
    ...
    entry = 'web/arena-lab/?actors=6&release=9.1'
    release = {'build':'C.09.1','source_repository':'mbace1/piritori-eden',
    ...
    data['index.html'] = ('<!doctype html><meta charset="utf-8"><title>Piritori C.09.1</title>'
                          f'<meta http-equiv="refresh" content="0;url={entry}"><a href="{entry}">Open arena</a>\n').encode()
    data['VERSIONS.md'] = (f'# C.09.1 — arena laboratory\n\nSource: {commit}.\n\n'
        'Retain frame timing remainder; update labels only when anchors or layout change. '
        '2/6/12-person stand-in fixtures, existing rules and character gates. '
        'Pixel/iPad acceptance remains pending.\n\n'
        '## Port\n\nGodot: retain target cadence under variable refresh; invalidate projected labels '
        'on camera, actor, text or viewport changes. No combat vector changes.\n').encode()
