...
- **No VOLUNTARY release path for a hired crew member.** `VERSIONS.md`
  v4.23 gave retirement a real, involuntary exit (the career ceiling,
  10 fights), so this is narrower than it was — a `crewStatus` of
  `'missing'` (v4.19's police-taken path) still has nowhere further to
  go, and there is still no way to choose to let someone go before they
  hit the ceiling. Godot has neither either, so both are faithful, not a
  corner cut.
