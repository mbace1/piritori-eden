...
| Clean M1 (replacement) | **Implemented and tested on `feat/m1-clean-inspection` @ `d0f6d6f` (Act I v4.52); not merged, not on the hub** | Inspection split from presence inside `app.js`, no overlay. New gate `web/test/m1-inspection.cjs` 63/63 (fails 11 on `652d704`); [gates 35993821372](https://github.com/mbace1/piritori-eden/actions/runs/35993821372) all 8 jobs green. Receipt, hub status and open gates: [design/M1_CLEAN_INSPECTION.md](design/M1_CLEAN_INSPECTION.md). Mark #91 superseded once this is reviewed. M2 not started. |
