...
export function mountCrew({content,button,replaceSession,getSession,refresh,run,tile}){
 ...
 function crewCard(p,ready,picked){
  const card=document.createElement('article');card.className='crew-card'+(picked&&ready?' picked':'')+(!ready?' unavailable':'');card.style.setProperty('--crew-color','#'+p.color.toString(16).padStart(6,'0'));
  ...
  else card.append(stamp(p.missing?'RECOVER':'RESTING',p.missing?'missing':'resting'));return card;
 }
 ...
 function paintBattle(b,u){
  ...
  for(const el of document.querySelectorAll('#actions [data-action]')){const type=el.dataset.action;el.hidden=type==='reload'&&!u.maxAmmo||type==='item'&&!!u.maxAmmo;if(!el.querySelector('svg')){el.prepend(icon(type==='attack'?u.equipment:type==='item'?'medical':type));el.append(text('small',type==='move'?'1 MOVE':'1 ACTION','command-cost'));}}
  $('actions').querySelector('[data-action=attack] svg')?.replaceWith(icon(u.equipment));
 ...
 function render(busy){
  ...
  if(menu){
   ...
   if(state.phase==='aftermath'){
    ...
    const target=rescueTarget(state);left.append(text('p','KARHUPUISTO / AFTER DARK','eyebrow'),text('h2',target?'Bring them home.':'Pick up the pieces.'),text('p',target?`${target.name} is stranded by the ${state.night%2?'north bench':'east path'}. Get them up. Bring the crew out through the south gate.`:'Recover the kit by the park path, then bring your crew home.','lead'));
    const note=document.createElement('div');note.className='route-note';note.append(icon(target?'help':'kit'),text('strong',target?'REACH → HELP → EXTRACT':'RECOVER → EXTRACT'),text('p','A way out matters more than a cleared park.'));left.append(note,rules());
