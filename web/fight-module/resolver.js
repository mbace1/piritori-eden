...
function enemyPhase(battle, onEvent = () => {}) {
  battle.phase = 'enemy';
  for (const enemy of battle.enemies.filter(item => item.alive)) {
    ...
    const target = targets.find(item => attackableInBattle(battle, enemy, item));
    if (!targets.length) break;
    // A missing legal attack is a reposition, never a hit through cover or
    // outside weapon reach. One reposition spends this unit's action.
    if (!target) {
      const old = enemy.cell;
      const from = parseSlotKey(old);
      const candidates = validMoveCells(battle, enemy).map(cell => {
        const candidate = { ...enemy, cell };
        const p = parseSlotKey(cell);
        return { cell, reachable: attackTargets(battle, candidate).length,
          distance: Math.abs(p.lane-from.lane)+Math.abs(p.depth-from.depth) };
      }).filter(p => p.reachable).sort((a,b)=>a.distance-b.distance || a.cell.localeCompare(b.cell));
      if (candidates.length) {
        enemy.cell = candidates[0].cell;
        battle.log.unshift(`${enemy.name} repositions ${describeCell(old)} → ${describeCell(enemy.cell)}.`);
        onEvent({type:'move',id:enemy.id,from:old,to:enemy.cell});
      } else {
        enemy.guard = Math.min(3, enemy.guard+1);
        battle.log.unshift(`${enemy.name} braces; no legal attack is available.`);
        onEvent({type:'brace',id:enemy.id});
      }
      continue;
    }
    onEvent({type:'attack',id:enemy.id,target:target.id});
...
export function autoCommand(battle, onEvent = () => {}) {
  ...
  for (const unit of battle.players.filter(item => item.alive && !battle.acted.includes(item.id))) {
    ...
    if (choice === 'attack') {
      onEvent({type:'attack',id:unit.id,target:bestTarget.id});
      battle.action = 'attack';
      playerAttack(battle, bestTarget.id);
    } else if (choice === 'move') {
      onEvent({type:'move',id:unit.id,from:unit.cell,to:reposition});
      battle.action = 'move';
      moveUnit(battle, reposition);
    } else {
      onEvent({type:'brace',id:unit.id});
      battle.action = 'brace';
      brace(battle);
    }
    if (battle.status !== 'active') break;
  }
  if (battle.status === 'active') enemyPhase(battle, onEvent);
  return true;
}
