...
export function makeStandIn(unit, world) {
  ...
  const prop=new T.Group();body.add(prop);const gun=unit.equipment.includes('handgun');
  const pm=new T.MeshStandardMaterial({color:gun?0x292f32:0x796348,roughness:.58,metalness:gun?.6:0});
  const weapon=new T.Mesh(gun?new T.BoxGeometry(.055,.085,.25):new T.CylinderGeometry(.027,.045,.61,8),pm);weapon.position.set(0,gun?.045:.20,gun?.10:.08);prop.add(weapon);prop.visible=false;
  ...
  a.play=(mode,duration=.8)=>{a.mode=mode;a.elapsed=0;a.duration=duration;a.down=mode==='down';prop.visible=['strike','shoot','grip'].includes(mode);};
  ...
  a.update=dt=>{
    ...
    const lean=a.mode==='strike'?pulse*.12:a.mode==='hit'?-pulse*.13:0;
    ...
    for(const side of [-1,1]){
      ...
      if(['shoot','grip'].includes(a.mode)){hand.set(side*.11,1.36,.50);}
      if(a.mode==='strike'&&side===-1){hand.set(-.20,1.26,.10+pulse*.44);}
      ...
      if(side===-1){prop.position.copy(hand);prop.rotation.set(a.mode==='strike'?-pulse*1.1:0,0,0);}
    }
