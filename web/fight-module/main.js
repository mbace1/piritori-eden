...
function layoutTags(){
  ...
  const obstacles=['roundbar','camera-tools','perf'].map(id=>{const b=$(id).getBoundingClientRect();return {x:b.x-rect.x,y:b.y-rect.y,width:b.width,height:b.height};});
...
function refresh(){
  if(!session||!ready)return;if(graphicsLost||layoutPaused){lockInput();return;}for(const el of document.querySelectorAll('#camera-tools button,#again'))el.disabled=false;const b=session.battle,u=selected(),ended=b.status!=='active',canAct=!busy&&!ended&&u?.alive&&!b.acted.includes(u.id);
...
function select(id){if(busy||graphicsLost||layoutPaused||!ready)return;session.command('select',id);action='';hint('Choose Attack, Move, Brace or Bandage.');refresh();}
...
async function run(type,value){if(busy||graphicsLost||layoutPaused||!ready)return;const result=session.command(type,value);if(!result.ok){hint(result.message||'That action is not available yet.');return;}const ticket=sequence;busy=true;action='';refresh();try{await present(result.record);}catch(e){if(!(e instanceof Interrupted))throw e;}if(ticket!==sequence||graphicsLost)return;busy=false;hint(session.battle.log[0]);refresh();if(auto&&session.battle.status==='active')autoTimer=setTimeout(()=>{if(auto&&!busy&&ticket===sequence)run('auto');},600);}
