...
import {createSession,checkpoint,restoreSession} from './session.js?v=2';
import {loadFighters,makeActor,updateActor,disposeActor} from './actors.js?v=2';
...
function saveRecovery(){try{sessionStorage.setItem(RECOVERY_KEY,JSON.stringify({fight:checkpoint(session),angle,zoom}));return true;}catch{return false;}}
function clearRecovery(){try{sessionStorage.removeItem(RECOVERY_KEY);}catch{}}
function lockInput(){for(const el of document.querySelectorAll('#panel button,#panel select,#camera-tools button,#again'))el.disabled=true;}
...
function fit(){if(!renderer)return;const w=area.clientWidth,h=area.clientHeight;if(!graphicsLost){const ratio=pixelRatioFor(profile,w,h,devicePixelRatio);if(renderer.getPixelRatio()!==ratio)renderer.setPixelRatio(ratio);if(canvas.width!==Math.floor(w*ratio)||canvas.height!==Math.floor(h*ratio))renderer.setSize(w,h,false);}const aspect=w/h,span=Math.max(10.9,13.8/aspect)/zoom;Object.assign(camera,{left:-span*aspect/2,right:span*aspect/2,top:span/2,bottom:-span/2});camera.position.set(Math.sin(angle)*13,10,Math.cos(angle)*13);camera.lookAt(0,.55,0);camera.updateProjectionMatrix();stage?.update(camera);}
function rebuildActors(){for(const a of actors.values())disposeActor(a);actors.clear();$('labels').replaceChildren();labels.clear();for(const u of all()){const a=makeActor(templates.get(u.modelId),u,world);a.group.position.copy(position(u.cell));a.group.rotation.y=u.side==='player'?Math.PI:0;if(!u.alive){a.play('down');a.elapsed=1;updateActor(a,0);}actors.set(u.id,a);const el=document.createElement('div');el.className='actor-label '+(u.side==='enemy'?'enemy':'');el.dataset.unit=u.id;$('labels').append(el);labels.set(u.id,el);}}
function reset(mode=$('loadout').value){if(graphicsLost||!ready)return;sequence++;auto=false;clearRecovery();session=createSession(content,mode);rebuildActors();busy=false;action='';$('result').hidden=true;hint('Choose a fighter, then an action. One action each round.');refresh();}
...
function refresh(){
  if(!session||!ready)return;if(graphicsLost){lockInput();return;}const b=session.battle,u=selected(),ended=b.status!=='active',canAct=!busy&&!ended&&u?.alive&&!b.acted.includes(u.id);
...
async function run(type,value){if(busy||graphicsLost||!ready)return;const result=session.command(type,value);if(!result.ok){hint(result.message||'That action is not available yet.');return;}const ticket=sequence;busy=true;action='';refresh();try{await present(result.record);}catch(e){if(!(e instanceof Interrupted))throw e;}if(ticket!==sequence||graphicsLost)return;busy=false;hint(session.battle.log[0]);refresh();if(auto&&session.battle.status==='active')setTimeout(()=>{if(auto&&!busy&&ticket===sequence)run('auto');},600);}
async function showcase(){if(busy||graphicsLost||!ready)return;const ticket=sequence;auto=false;busy=true;action='';refresh();const starts=new Map([...actors].map(([id,a])=>[id,a.group.position.clone()]));try{for(const mode of ['idle','walk','strike','shoot','brace','item','talk','hit','grip','down']){hint(`ACTION REHEARSAL · ${mode.toUpperCase()} · ${['idle','walk'].includes(mode)?'own-character clip':'prototype gesture'}`);for(const a of actors.values()){a.down=false;a.play(mode,1.15);}await tween(1.15,t=>{if(mode==='walk')for(const [id,a] of actors)a.group.position.copy(starts.get(id)).add(new T.Vector3(.45*Math.sin(t*Math.PI),0,0));});}}catch(e){if(!(e instanceof Interrupted))throw e;}if(ticket!==sequence||graphicsLost)return;reset();hint('Rehearsal complete. Fresh battle ready.');}
...
function bindGraphicsRecovery(){
  ...
  canvas.addEventListener('webglcontextrestored',()=>{
    ...
    applyProfile();previous=performance.now();frameCount=seconds=0;
