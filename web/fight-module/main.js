...
import {buildKarhupuisto} from '../bear-path/park.js?v=1';
import {createEncounter} from '../bear-path/encounter.js?v=1';
import {mountBearPath} from '../bear-path/presentation.js?v=1';
...
let story=null,cameraPullback=false;
...
function buildStage(){
  ...
  stage=(isBear?buildKarhupuisto:buildNightCourtyard)(world,renderer,session.battle.cover,position);
...
function refresh(){
  if(!session||!ready)return;if(graphicsLost||layoutPaused){lockInput();return;}for(const el of document.querySelectorAll('#camera-tools button,#again'))el.disabled=false;if(story){story.render();if(!story.isBattle())return;}const b=session.battle,u=selected(),ended=b.status!=='active',canAct=!busy&&!ended&&u?.alive&&!b.acted.includes(u.id);
...
function tick(now){raf=0;if(hidden||graphicsLost||!ready)return;queueFrame();const real=Math.max(0,(now-previous)/1000);if(real<1/profile.fps-.001)return;previous=now;const dt=Math.min(.05,real);for(const a of actors.values())updateActor(a,layoutPaused?0:dt);if(!story||story.isBattle())layoutTags();story?.layout();stage?.tick?.(layoutPaused||matchMedia('(prefers-reduced-motion: reduce)').matches?0:dt);renderer.render(world,camera);edgeSmoothing.render(profile.edgeSmoothing);controller(now);frameCount++;seconds+=real;if(seconds>2){fps=Math.round(frameCount/seconds);if(fps<22&&!low){low=true;profile=renderProfile({touch:true});applyProfile();}$('perf').textContent=`${isBear?'C.07':'C.06'} · ${fps} FPS · ${renderer.info.render.calls} draws · ${profile.name}`;frameCount=0;seconds=0;}}
async function boot(){
