...
The mobile/recovered scene stays within 650,000 drawing-buffer pixels, FXAA, no realtime shadows, and 1024² character/ground texture uploads. Measurements in desktop-host Edge are functional evidence, not Pixel 10 Pro or iPad M2 frame-rate claims. The follow-up audit identified AMD/ANGLE D3D11 rendering; allowing SwiftShader fallback did not establish software rendering. Tested style-recovery frames use 25 tracked geometries and 12 textures; draw calls vary by viewport/phase and were 55–56 in battle. Environment transfer is 3,785,540 bytes, in addition to existing fighters/vendor code.

The [follow-up 3D audit](BEAR_PATH_3D_AUDIT.md) found severe rotated-camera foliage occlusion, walking ground-contact errors and ground penetration during knockdown. Functional success here does not imply visual acceptance.
...
The first public C.08 smoke test caught a deployment-only error: the source register's relative fighter paths replaced the hub's immutable raw-GitHub URLs, producing a fighter 404. The corrected hub register contains the same two fighter IDs/bytes/hashes at their original pinned URLs plus the two new local environment assets. The entry/manifest cache tokens were advanced together. This is a deployment mapping correction, not replacement character art. Use `tools/publish/bear_path_manifest.py` with the previously published hub register; its regression test rejects changed fighter identities and moving branch URLs. Never publish the source's entire art register over the hub derivative without this mapping.
