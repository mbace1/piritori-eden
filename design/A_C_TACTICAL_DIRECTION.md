...
Owner direction, 2026-09-13: the TURF summary applies to **A (2D Turf)** and **C (the active 3D route)**. Follow-up: **"it's Mewgenics meets Xcom."** This is a design update, not evidence of implemented parity. C.09.1 remains the active public lab.
...
1. **One Move and one Action per unit, either order.** Also permit spending only one. Reload spends Action, preserving Move. C.09.1 currently spends its single action on repositioning: that is an implementation gap, not the new design target. This replaces the previous C.10 plan's deferral of Move + Act as an optional later experiment.
...
| --- | --- | --- |
| Tactical design | Existing Move + Act, forecasts and intent are the reference to inspect | Implement the shared contract, with explicit migration from C.09.1 |
...
**Next C batch:** Move + Act and honest intents/forecasts, legal movement plus both cover classes, weapon-specific readable animation, then directed camera and the practical-light/material pass on the same arena. Test the entire sequence together. The C.10 plan is the implementation breakdown; it is not shipped.
